...
def parse_args():
    ...
    parser.add_argument('--noSNG', action='store_true')
    parser.add_argument('--PN', action='store_true')
    parser.add_argument('--EQLR', action='store_true')
    parser.add_argument('--noSCHED', action='store_true')

    parser.add_argument('--realm', action='store_true', help='Include KL of the training samples in the loss function, when using margin')
    parser.add_argument('--boostreco', type=int, default=0, help='Increase L1 loss weight after 32x32 reso at every phase shift by this factor.')
    ...
    parser.add_argument('-d', '--data', default='celeba', type=str,
                        choices=['celeba', 'lsun', 'cifar10', 'celebaHQ','ffhq'],
    ...
    parser.add_argument('--mgpu', action='store_true', help='Force multi-GPU')
    ...
    parser.add_argument('--m', type=float, default=0.0, help='KL margin')
    parser.add_argument('--e_last_relu', action='store_true', help='Use LReLU on the last layer of the encoder')
...
def init():
    ...
    args.h5 = (not (args.test_path is None) and args.test_path[-3:] == '.h5') or ( not (args.train_path is None) and args.train_path[-3:] == '.h5' )
    ...
    print("Will train up to {}k samples. Max phase for dataset {} is {}. Once the maximum phase is trained the full round, we continue training that phase.".format(args.total_kimg, args.data, args.max_phase))

    import os
    if args.start_iteration==-1:
        cppath = args.save_dir + '/checkpoint'
        if os.path.exists(cppath):
            print('exists {}'.format(cppath))
